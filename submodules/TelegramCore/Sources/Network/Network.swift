...
private class MTProtoConnectionStatusDelegate: NSObject, MTProtoDelegate {
  var action: (MTProtoConnectionInfo) -> Void = { _ in }
  let info = Atomic<MTProtoConnectionInfo>(
    value: MTProtoConnectionInfo(flags: [], proxyAddress: nil))

  @objc func mtProtoNetworkAvailabilityChanged(_ mtProto: MTProto!, isNetworkAvailable: Bool) {
    self.action(
      self.info.modify { info in
        var info = info
  ...
  @objc func mtProtoConnectionContextUpdateStateChanged(
    _ mtProto: MTProto!, isUpdatingConnectionContext: Bool
  ) {
    self.action(
      self.info.modify { info in
        var info = info
...
private func usageCalculationInfo(basePath: String, category: MediaResourceStatsCategory?)
  -> MTNetworkUsageCalculationInfo
{
  ...
  return MTNetworkUsageCalculationInfo(
    filePath: basePath + "/network-stats",
    incomingWWANKey: UsageCalculationTag(
      connection: .cellular, direction: .incoming, category: categoryValue
    ).key,
    outgoingWWANKey: UsageCalculationTag(
      connection: .cellular, direction: .outgoing, category: categoryValue
    ).key,
    incomingOtherKey: UsageCalculationTag(
      connection: .wifi, direction: .incoming, category: categoryValue
    ).key,
    outgoingOtherKey: UsageCalculationTag(
      connection: .wifi, direction: .outgoing, category: categoryValue
    ).key)
}
...
public struct NetworkUsageStatsDirectionsEntry: Equatable {
  ...
  public static func == (
    lhs: NetworkUsageStatsDirectionsEntry, rhs: NetworkUsageStatsDirectionsEntry
  ) -> Bool {
...
public struct NetworkUsageStatsConnectionsEntry: Equatable {
  ...
  public static func == (
    lhs: NetworkUsageStatsConnectionsEntry, rhs: NetworkUsageStatsConnectionsEntry
  ) -> Bool {
...
private func interfaceForConnection(_ connection: UsageCalculationConnection)
  -> MTNetworkUsageManagerInterface
{
  return MTNetworkUsageManagerInterface(rawValue: UInt32(connection.rawValue))
}

func updateNetworkUsageStats(
  basePath: String, category: MediaResourceStatsCategory, delta: NetworkUsageStatsConnectionsEntry
) {
  let info = usageCalculationInfo(basePath: basePath, category: category)
  let manager = MTNetworkUsageManager(info: info)!

  manager.addIncomingBytes(
    UInt(clamping: delta.wifi.incoming), interface: interfaceForConnection(.wifi))
  manager.addOutgoingBytes(
    UInt(clamping: delta.wifi.outgoing), interface: interfaceForConnection(.wifi))

  manager.addIncomingBytes(
    UInt(clamping: delta.cellular.incoming), interface: interfaceForConnection(.cellular))
  manager.addOutgoingBytes(
    UInt(clamping: delta.cellular.outgoing), interface: interfaceForConnection(.cellular))
}

func networkUsageStats(basePath: String, reset: ResetNetworkUsageStats) -> Signal<
  NetworkUsageStats, NoError
> {
  return
    ((Signal<NetworkUsageStats, NoError> { subscriber in
      ...
      if reset.contains(.wifi) {
        ...
        resetAddKeys[UsageCalculationResetKey.wifi.rawValue as NSNumber] =
          Int64(timestamp) as NSNumber
      }
      if reset.contains(.cellular) {
        resetKeys += rawKeys.filter({ $0.connection == .cellular }).map({ $0.key as NSNumber })
        resetAddKeys[UsageCalculationResetKey.cellular.rawValue as NSNumber] =
          Int64(timestamp) as NSNumber
      }
          ...
          NetworkUsageStats(
            generic: NetworkUsageStatsConnectionsEntry(
              cellular: NetworkUsageStatsDirectionsEntry(
                incoming: dict[
                  UsageCalculationTag(
                    connection: .cellular, direction: .incoming, category: .generic
                  ).key]!,
                outgoing: dict[
                  UsageCalculationTag(
                    connection: .cellular, direction: .outgoing, category: .generic
                  ).key]!),
            ...
            image: NetworkUsageStatsConnectionsEntry(
              ...
              wifi: NetworkUsageStatsDirectionsEntry(
                incoming: dict[
                  UsageCalculationTag(connection: .wifi, direction: .incoming, category: .image).key
                ]!,
                outgoing: dict[
                  UsageCalculationTag(connection: .wifi, direction: .outgoing, category: .image).key
                ]!)),
            video: NetworkUsageStatsConnectionsEntry(
              ...
              wifi: NetworkUsageStatsDirectionsEntry(
                incoming: dict[
                  UsageCalculationTag(connection: .wifi, direction: .incoming, category: .video).key
                ]!,
                outgoing: dict[
                  UsageCalculationTag(connection: .wifi, direction: .outgoing, category: .video).key
                ]!)),
            audio: NetworkUsageStatsConnectionsEntry(
              ...
              wifi: NetworkUsageStatsDirectionsEntry(
                incoming: dict[
                  UsageCalculationTag(connection: .wifi, direction: .incoming, category: .audio).key
                ]!,
                outgoing: dict[
                  UsageCalculationTag(connection: .wifi, direction: .outgoing, category: .audio).key
                ]!)),
            file: NetworkUsageStatsConnectionsEntry(
              ...
              wifi: NetworkUsageStatsDirectionsEntry(
                incoming: dict[
                  UsageCalculationTag(connection: .wifi, direction: .incoming, category: .file).key]!,
                outgoing: dict[
                  UsageCalculationTag(connection: .wifi, direction: .outgoing, category: .file).key]!
              )),
            call: NetworkUsageStatsConnectionsEntry(
              ...
              wifi: NetworkUsageStatsDirectionsEntry(
                incoming: dict[
                  UsageCalculationTag(connection: .wifi, direction: .incoming, category: .call).key]!,
                outgoing: dict[
                  UsageCalculationTag(connection: .wifi, direction: .outgoing, category: .call).key]!
              )),
            sticker: NetworkUsageStatsConnectionsEntry(
              cellular: NetworkUsageStatsDirectionsEntry(
                incoming: dict[
                  UsageCalculationTag(
                    connection: .cellular, direction: .incoming, category: .stickers
                  ).key]!,
                outgoing: dict[
                  UsageCalculationTag(
                    connection: .cellular, direction: .outgoing, category: .stickers
                  ).key]!),
              wifi: NetworkUsageStatsDirectionsEntry(
                incoming: dict[
                  UsageCalculationTag(connection: .wifi, direction: .incoming, category: .stickers)
            ...
            voiceMessage: NetworkUsageStatsConnectionsEntry(
              cellular: NetworkUsageStatsDirectionsEntry(
                ...
                outgoing: dict[
                  UsageCalculationTag(
                    connection: .cellular, direction: .outgoing, category: .voiceMessages
                  ).key]!),
              wifi: NetworkUsageStatsDirectionsEntry(
                incoming: dict[
                  UsageCalculationTag(
                    connection: .wifi, direction: .incoming, category: .voiceMessages
                  ).key]!,
                outgoing: dict[
                  UsageCalculationTag(
                    connection: .wifi, direction: .outgoing, category: .voiceMessages
                  ).key]!)),
    ...
    |> then(
      Signal<NetworkUsageStats, NoError>.complete()
        |> delay(5.0, queue: Queue.concurrentDefaultQueue()))) |> restart
}
...
public struct NetworkInitializationArguments {
  ...
  public init(
    apiId: Int32, apiHash: String, languagesCategory: String, appVersion: String,
    voipMaxLayer: Int32, voipVersions: [CallSessionManagerImplementationVersion],
    appData: Signal<Data?, NoError>,
    externalRequestVerificationStream: Signal<[String: String], NoError>,
    externalRecaptchaRequestVerification: @escaping (String, String) -> Signal<String?, NoError>,
    autolockDeadine: Signal<Int32?, NoError>, encryptionProvider: EncryptionProvider,
    deviceModelName: String?, useBetaFeatures: Bool, isICloudEnabled: Bool
  ) {
    self.apiId = apiId
...
func initializedNetwork(
  accountId: AccountRecordId, arguments: NetworkInitializationArguments, supplementary: Bool,
  datacenterId: Int, keychain: Keychain, basePath: String, testingEnvironment: Bool,
  languageCode: String?, proxySettings: ProxySettings?, networkSettings: NetworkSettings?,
  phoneNumber: String?, useRequestTimeoutTimers: Bool, appConfiguration: AppConfiguration
) -> Signal<Network, NoError> {
  return Signal { subscriber in
    ...
    queue.async {
      ...
      if let effectiveActiveServer = proxySettings?.effectiveActiveServer {
        apiEnvironment = apiEnvironment.withUpdatedSocksProxySettings(
          effectiveActiveServer.mtProxySettings)
      }

      apiEnvironment = apiEnvironment.withUpdatedNetworkSettings(
        (networkSettings ?? NetworkSettings.defaultSettings).mtNetworkSettings)
      ...
      let context = MTContext(
        serialization: serialization, encryptionProvider: arguments.encryptionProvider,
        apiEnvironment: apiEnvironment, isTestingEnvironment: testingEnvironment,
        useTempAuthKeys: useTempAuthKeys)
        ...
        if useNetworkFramework {
          if #available(iOS 12.0, macOS 14.0, *) {
            context.makeTcpConnectionInterface = { delegate, delegateQueue in
              return NetworkFrameworkTcpConnectionInterface(
      ...
      for (id, ips) in seedAddressList {
        context.setSeedAddressSetForDatacenterWithId(
          id,
          seedAddressSet: MTDatacenterAddressSet(
            addressList: ips.map {
              MTDatacenterAddress(
                ip: $0,
                port: 2398,
      ...
      var wrappedAdditionalSource: MTSignal?
        ...
        if #available(iOS 10.0, *), !supplementary, arguments.isICloudEnabled {
          ...
          if let value = cloudDataContext.with({ $0 }) {
            ...
            cloudDataContextValue = makeCloudDataContext(
              encryptionProvider: arguments.encryptionProvider)
          ...
          if let cloudDataContext = cloudDataContextValue {
            wrappedAdditionalSource = MTSignal(generator: { subscriber in
              let disposable = cloudDataContext.get(phoneNumber: .single(phoneNumber)).start(
                next: { value in
      ...
      if !supplementary {
        ...
        context.setExternalRequestVerification({ nonce in
          return MTSignal(generator: { subscriber in
            let disposable =
              (externalRequestVerificationStream
              |> map { dict -> String? in
                return dict[nonce]
              }
              |> filter { $0 != nil }
              |> take(1)
              |> timeout(15.0, queue: .mainQueue(), alternate: .single("APNS_PUSH_TIMEOUT"))).start(
                next: { secret in
        ...
        context.setExternalRecaptchaRequestVerification({ method, siteKey in
          return MTSignal(generator: { subscriber in
            let disposable =
              (externalRecaptchaRequestVerification(method, siteKey)
              |> filter { $0 != nil }
              |> take(1)
              |> timeout(15.0, queue: .mainQueue(), alternate: .single("RECAPTCHA_TIMEOUT"))).start(
                next: { token in
      ...
      let mtProto = MTProto(
        ...
        usageCalculationInfo: usageCalculationInfo(basePath: basePath, category: nil),
      ...
      connectionStatusDelegate.action = { [weak connectionStatus] info in
        if info.flags.contains(.Connected) {
          ...
          } else if !info.flags.intersection([.UpdatingConnectionContext, .PerformingServiceTasks])
            .isEmpty
          {
            connectionStatus?.set(.single(.updating(proxyAddress: info.proxyAddress)))
          } else {
      ...
      if let data = appConfiguration.data, data["ios_killswitch_disable_downloadv2"] != nil {
        useExperimentalFeatures = false
      }

      let network = Network(
        queue: queue, datacenterId: datacenterId, context: context, mtProto: mtProto,
        requestService: requestService, connectionStatusDelegate: connectionStatusDelegate,
        _connectionStatus: connectionStatus, basePath: basePath,
        appDataDisposable: appDataDisposable, encryptionProvider: arguments.encryptionProvider,
        useRequestTimeoutTimers: useRequestTimeoutTimers,
        useBetaFeatures: arguments.useBetaFeatures, useExperimentalFeatures: useExperimentalFeatures
      )

      if let data = appConfiguration.data,
        let notifyInterval = data["upload_premium_speedup_notify_period"] as? Double
      {
...
private final class NetworkHelper: NSObject, MTContextChangeListener {
  ...
  init(
    requestPublicKeys: @escaping (Int) -> Signal<NSArray, NoError>,
    isContextNetworkAccessAllowed: @escaping () -> Signal<Bool, NoError>,
    contextProxyIdUpdated: @escaping (NetworkContextProxyId?) -> Void,
    contextLoggedOutUpdated: @escaping () -> Void
  ) {
...
extension NetworkContextProxyId {
  fileprivate init?(settings: MTSocksProxySettings) {
...
public final class Network: NSObject, MTRequestMessageServiceDelegate {
  ...
  private let networkSpeedLimitedEventState = Atomic<NetworkSpeedLimitedEventState>(
    value: NetworkSpeedLimitedEventState())
  ...
  fileprivate init(
    queue: Queue, datacenterId: Int, context: MTContext, mtProto: MTProto,
    requestService: MTRequestMessageService,
    connectionStatusDelegate: MTProtoConnectionStatusDelegate,
    _connectionStatus: Promise<ConnectionStatus>, basePath: String, appDataDisposable: Disposable,
    encryptionProvider: EncryptionProvider, useRequestTimeoutTimers: Bool, useBetaFeatures: Bool,
    useExperimentalFeatures: Bool
  ) {
    self.encryptionProvider = encryptionProvider
    ...
    self._contextProxyId = ValuePromise(
      (context.apiEnvironment.socksProxySettings as MTSocksProxySettings?).flatMap(
        NetworkContextProxyId.init(settings:)), ignoreRepeated: true)
    ...
    let networkHelper = NetworkHelper(
      requestPublicKeys: { [weak self] id in
          ...
          return strongSelf.request(Api.functions.help.getCdnConfig())
            ...
            |> map { result -> NSArray in
              ...
              if let result = result {
                switch result {
                case .cdnConfig(let cdnConfigData):
                  let publicKeys = cdnConfigData.publicKeys
                  for key in publicKeys {
                    switch key {
                    case .cdnPublicKey(let cdnPublicKeyData):
    ...
    self._multiplexedRequestManager = MultiplexedRequestManager(takeWorker: {
      [weak self] target, tag, continueInBackground in
      if let strongSelf = self {
        ...
        if datacenterId != 0 {
          return strongSelf.makeWorker(
            datacenterId: datacenterId, isCdn: isCdn, isMedia: isMedia, tag: tag,
    ...
    let shouldKeepConnectionSignal =
      self.shouldKeepConnection.get()
      |> distinctUntilChanged |> deliverOn(queue)
    self.shouldKeepConnectionDisposable.set(
      shouldKeepConnectionSignal.start(next: { [weak self] value in
        if let strongSelf = self {
          if value {
  ...
  public func requestMessageServiceAuthorizationRequired(
    _ requestMessageService: MTRequestMessageService!
  ) {
    Logger.shared.log("Network", "requestMessageServiceAuthorizationRequired")
    self.loggedOut?()
  }

  func download(datacenterId: Int, isMedia: Bool, isCdn: Bool = false, tag: MediaResourceFetchTag?)
    -> Signal<Download, NoError>
  {
  ...
  private func makeWorker(
    datacenterId: Int, isCdn: Bool, isMedia: Bool, tag: MediaResourceFetchTag?,
    continueInBackground: Bool = false
  ) -> Download {
    let queue = Queue.mainQueue()
    let shouldKeepWorkerConnection: Signal<Bool, NoError> =
      combineLatest(
        queue: queue, self.shouldKeepConnection.get(),
        self.shouldExplicitelyKeepWorkerConnections.get(),
        self.shouldKeepBackgroundDownloadConnections.get())
      |> map {
        shouldKeepConnection, shouldExplicitelyKeepWorkerConnections,
        shouldKeepBackgroundDownloadConnections -> Bool in
        return shouldKeepConnection || shouldExplicitelyKeepWorkerConnections
          || (continueInBackground && shouldKeepBackgroundDownloadConnections)
      }
      |> distinctUntilChanged
    return Download(
      queue: self.queue, datacenterId: datacenterId, isMedia: isMedia, isCdn: isCdn,
      context: self.context, masterDatacenterId: self.datacenterId,
      usageInfo: usageCalculationInfo(
        basePath: self.basePath, category: (tag as? TelegramMediaResourceFetchTag)?.statsCategory),
      shouldKeepConnection: shouldKeepWorkerConnection,
      useRequestTimeoutTimers: self.useRequestTimeoutTimers)
  }

  private func worker(datacenterId: Int, isCdn: Bool, isMedia: Bool, tag: MediaResourceFetchTag?)
    -> Signal<Download, NoError>
  {
    return Signal { [weak self] subscriber in
      if let strongSelf = self {
        subscriber.putNext(
          strongSelf.makeWorker(
            datacenterId: datacenterId, isCdn: isCdn, isMedia: isMedia, tag: tag))
      }
      subscriber.putCompletion()
  ...
  public func mergeBackupDatacenterAddress(
    datacenterId: Int32, host: String, port: Int32, secret: Data?
  ) {
    self.context.performBatchUpdates {
      let address = MTDatacenterAddress(
        ip: host, port: UInt16(port), preferForMedia: false, restrictToTcp: false, cdn: false,
        preferForProxy: false, secret: secret)
      ...
      let currentSchemes = self.context.transportSchemesForDatacenter(
        withId: Int(datacenterId), media: false, enforceMedia: false, isProxy: false)
      var found = false
      ...
      if !found {
        let scheme = MTTransportScheme(
          transport: MTTcpTransport.self, address: address, media: false)
        self.context.updateTransportSchemeForDatacenter(
          withId: Int(datacenterId), transportScheme: scheme, media: false, isProxy: false)
      }
    }
  }
  ...
  public func getAuthKeyId() -> Signal<Int64, NoError> {
    ...
    return Signal { subscriber in
      MTContext.contextQueue().dispatch(onQueue: {
        var result: Int64 = 0
        if let authInfo = mtContext?.authInfoForDatacenter(
          withId: datacenterId, selector: .persistent)
        {
  ...
  public func requestWithAdditionalInfo<T>(
    _ data: (FunctionDescription, Buffer, DeserializeFunctionResponse<T>),
    info: NetworkRequestAdditionalInfo, tag: NetworkRequestDependencyTag? = nil,
    automaticFloodWait: Bool = true, onFloodWaitError: ((String) -> Void)? = nil
  ) -> Signal<NetworkRequestResult<T>, MTRpcError> {
    ...
    return Signal { subscriber in
      ...
      request.setPayload(
        ...
        shortMetadata: WrappedRequestShortMetadata(
          shortMetadata: WrappedShortFunctionDescription(data.0)),
      ...
      request.shouldContinueExecutionWithErrorContext = { errorContext in
        ...
        if let onFloodWaitError, errorContext.floodWaitSeconds > 0,
          let errorText = errorContext.floodWaitErrorText
        {
      ...
      request.completed = { (boxedResponse, timestamp, error) -> Void in
        if let error = error {
          ...
          if let result = (boxedResponse as! BoxedMessage).body as? T {
            ...
            subscriber.putError(
              MTRpcError(errorCode: 500, errorDescription: "TL_VERIFICATION_ERROR"))
          }
        }
      }
  ...
  public func request<T>(
    _ data: (FunctionDescription, Buffer, DeserializeFunctionResponse<T>),
    tag: NetworkRequestDependencyTag? = nil, automaticFloodWait: Bool = true,
    onFloodWaitError: ((String) -> Void)? = nil
  ) -> Signal<T, MTRpcError> {
    let requestService = self.requestService
    return Signal { subscriber in
      let request = MTRequest()
...
class Keychain: NSObject, MTKeychain {
  ...
  init(
    get: @escaping (String) -> Data?, set: @escaping (String, Data) -> Void,
